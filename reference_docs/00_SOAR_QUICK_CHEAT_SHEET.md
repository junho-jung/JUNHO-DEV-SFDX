...
1. **감지 (Detection)**: `EM_` 인터셉터가 이벤트를 캡처하여 `SecurityGuard`(Facade)로 넘깁니다.
2. **이벤트 추출 (Extraction)**: `SecurityAlertPublisher`가 `IEventExtractor` 전략 패턴을 기반으로 이벤트 페이로드를 규격화하여 발행(`SecurityAlert__e`)합니다.
3. **정책 평가 (SecurityPolicy__mdt)**: `SecurityMetadataRegistry`(Singleton)를 통해 로드된 정책에 따라, 위협 발생 횟수가 `Threshold(임계치)`를 넘었는지 평가하고 해당 `ActionType`(예: `NOTIFY_TEAMS`)을 결정합니다.
4. **연동 설정 (SecurityWebhookConfig__mdt)**: 순수 관리형 패키지 모드에서는 외부 API(슬랙, 팀즈 등) 통신용 Endpoint 혹은 **Named Credential**을 조회합니다. (레거시 `Interface__c`에 종속되지 않습니다.)
5. **동적 팩토리 실행 (Execution)**: `SecurityActionFactory`가 리플렉션(`Type.forName`)을 통해 액션 구현체(예: `SecurityNotifyTeamsAction.cls`)를 동적 생성하고 실행한 뒤 결과를 고유 상태 로그(`SecurityActionLog__c`)에 기록합니다.
...
### 🔗 [케이스 2] 순수 웹훅(Webhook) 발송 인증(Named Credential) 변경
* "슬랙이나 팀즈 등 자체 SOAR 웹훅으로 알람을 쏘고 싶을 때 (레거시 Bridge 비사용)"
1. Salesforce 설정에서 새 대상 시스템용 **Named Credential** 생성 (예: `IF_Teams_Base`)
2. `SecurityWebhookConfig__mdt` 에서 알림용 레코드를 열어 **Endpoint__c** 필드값을 `callout:IF_Teams_Base`로 치환합니다. (코드 배포 없이 즉시 외부 전송 토큰 전환 완료)
...
```apex
// ISecurityAction 인터페이스를 구현 (또는 SecurityBaseAction/SecurityBaseWebhookAction 상속)
public with sharing class SecurityCustomERPBlockAction extends SecurityBaseAction {
    protected override void doExecute(Map<String, Object> payloadMap, String targetUserId, String originalPayloadJson) {
        ...
        SecuritySoarTrace.log('SecurityCustomERPBlockAction.doExecute', new Map<String, Object>{ 'UserId' => targetUserId });
    }
}
```
