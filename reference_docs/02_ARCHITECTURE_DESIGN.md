...
graph TD
    ...
    BaseInt ==>|이벤트 Facade 델리게이팅| Guard(SecurityGuard)
    Guard ==>|이벤트 발행| Pub(SecurityAlertPublisher)
    Pub -->|IEventExtractor 전략| AlertE((("SecurityAlert__e")))
    ...
    subgraph 3. Execution Engine
        ...
        Fact -->|Instantiate| BWeb[SecurityBaseWebhookAction]
        
        Hand -->|Submit Job| Exec[SecurityActionExecutor]
        Exec -->|"executeWithLogging\nSync / Async Job"| BAct
        Exec -->|"executeWithLogging\nQueueable"| BWeb
    end
    ...
    subgraph 4. Concrete Action Components
        ...
        BWeb -.-> MDT_W[("SecurityWebhookConfig__mdt\n(Endpoint, IsActive)")]
        BWeb --> Act_Slk["SecurityNotifySlackAction"]
        BWeb --> Act_Tms["SecurityNotifyTeamsAction\n(TeamsPayloadBuilder)"]
    end
...
### 2.3 SecurityWebhookConfig__mdt (독립형 웹훅 아웃바운드 규격 테이블 - New)
내부 SOAR 웹훅 액션(`SecurityNotifySlackAction`, `SecurityNotifyTeamsAction`)이 호출할 콜아웃 Endpoint(Named Credential 등)와 활성화 상태를 관리합니다. 기존 `SecurityIntegration__mdt` 인프라를 버리고 순수 SOAR 전용으로 신설되었습니다.
* **주요 필드**
  * `ActionType__c`: `NOTIFY_SLACK` 등 액션 식별자
  * `Endpoint__c`: `callout:IF_Teams_Base` 등 실제 통신 목적지
  * `IsActive__c`: 알림 발송 On/Off 스위치

### 2.4 SecurityIntegration__mdt (레거시 아웃바운드 규격 테이블 - Deprecated in Core)
오로지 `SecurityInterfaceBridge.cls`를 통해 기존 통신 프레임워크(`Interface__c`)를 우회 호출할 때만 사용되는 구형 메타데이터입니다. 순수 SOAR 관리형 패키징 시에는 배제됩니다.
* **주요 필드**
  * `InterfaceId__c`: `IF-001` 등 레거시 프레임워크의 상세 설정 매핑 Key
  * `Mode__c`: 비동기 연동 스레드 모드 지정 (`QUEUEABLE` 등)

이러한 설계 구조를 통해 "탐지(Interceptor) ➞ 퍼사드(SecurityGuard) ➞ 전략기반 추출(Publisher) ➞ 캐시조회(Registry) ➞ 동적 할당(Factory) ➞ 플라이웨이트 실행(Webhook Base)" 이라는 최고수준의 OCP(개방-폐쇄) 아키텍처가 완성되었습니다.
