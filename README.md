...
### 🌟 메인 진행 문서 (Main Documentation)
현재 진행 중인 리팩토링 및 패키징 핵심 설계와 결과를 담은 리소스입니다. (`docs/` 디렉토리)
...
#### [📦 02. 패키징 독립성 확보 및 메타데이터 전략 (Tier 2 Packaging Strategy)](docs/02_TIER2_PACKAGING_STRATEGY.md)
...
#### [📦 05. SOAR 프레임워크 패키징 컴포넌트 전수 레이블링 (Comprehensive Component Labeling)](docs/05_SOAR_PACKAGE_COMPONENTS.md)
* **주요 내용**: 현재 환경에 존재하는 수많은 오브젝트(Object), 플랫폼 이벤트, 커스텀 메타데이터 중 순수 SOAR 요소만을 발라내어, 향후 독립 패키지에 포함시킬지 여부(O/X)와 테스트 대상 범위를 완전히 분리하는 가이드입니다.
* **유용한 대상**: 패키징 빌드 담당자, 테스트 코드 작성자, 프로젝트 매니저

---

### 📚 참조 문서 (Reference Documentation)
초기 아키텍처 설계, 기획, 그리고 향후 고도화를 위한 심층 분석 자료입니다. (`reference_docs/` 디렉토리)
...
* **주요 내용**: 전체 소스 코드를 단일 단일 덩어리(Monolithic)가 아닌, 공통 API 통신 뼈대인 `Base-Interface` 패키지와 보안 특화 `Security-SOAR` 패키지로 수평 분할(Split)하고 `sfdx-project.json`으로 의존성을 주입하는 모듈형 아키텍처 전략을 다룹니다.
